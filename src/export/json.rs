...
use super::{ExportData, ExportMetadata, UniversalExporter};
...
impl JsonExporter {
    ...
    pub fn export_web_to_file<P: AsRef<Path>>(
        &self,
        path: P,
        options: &ExportOptions,
    ) -> Result<(), JsonExportError> {
    ...
    pub fn stream_export<W: Write>(
        ...
        writeln!(
            buf_writer,
            "  \"project\": {},",
            serde_json::to_string(&export_data.project)?
        )?;
        writeln!(
            buf_writer,
            "  \"metrics\": {},",
            serde_json::to_string(&export_data.metrics)?
        )?;
        ...
        for (i, chunk) in export_data.symbols.chunks(chunk_size).enumerate() {
            ...
            if options.streaming.include_progress {
                ...
                let _progress =
                    ((i + 1) * chunk_size).min(total_symbols) as f32 / total_symbols as f32 * 100.0;
            }
        }
        ...
        writeln!(
            buf_writer,
            "  \"files\": {},",
            serde_json::to_string(&export_data.files)?
        )?;
        writeln!(
            buf_writer,
            "  \"export_info\": {}",
            serde_json::to_string(&export_data.export_info)?
        )?;
        writeln!(buf_writer, "}}")?;
    ...
    fn convert_to_web_format(&self, export_data: ExportData) -> Result<WebExport, JsonExportError> {
        let symbols: Vec<WebSymbol> = export_data
            .symbols
            .into_iter()
            .map(|s| WebSymbol {
                id: s.id,
        ...
        let relationships: Vec<WebRelationship> = export_data
            .relationships
            .into_iter()
        ...
        let file_index: Vec<WebFileInfo> = export_data
            .files
            .into_iter()
            .map(|f| WebFileInfo {
                path: f.path,
        ...
        let summary =
            self.create_export_summary(&symbols, &relationships, &file_index, &export_data.metrics);
    ...
    fn create_export_summary(
        ...
        let complexity_buckets: Vec<ComplexityBucket> = metrics
        ...
        let mut top_files: Vec<TopFileInfo> = files
            .iter()
            .map(|f| TopFileInfo {
                path: f.path.clone(),
                symbol_count: f.symbol_count,
        ...
        let mut top_symbols: Vec<TopSymbolInfo> = symbols
            .iter()
            .map(|s| TopSymbolInfo {
                name: s.name.clone(),
