    ...
    if path_buf.is_dir() {
        ...
        if let Err(e) = std::fs::read_dir(&path_buf) {
            return Err(ValidationError::PermissionDenied(format!(
                "Permission denied when reading directory '{}': {}",
                path, e
            )));
        }
...
pub fn safe_resolve_path(base: &Path, path: &str) -> ValidationResult<PathBuf> {
            ...
            if let Some(parent) = resolved.parent() {
                match parent.canonicalize() {
                    Ok(normalized_parent) => {
                        normalized_parent.join(resolved.file_name().unwrap_or_default())
                    }
                    Err(_) => {
                        return Err(ValidationError::InvalidPath(format!(
    ...
    if let Ok(base_normalized) = base.canonicalize() {
        if !normalized.starts_with(&base_normalized) {
            return Err(ValidationError::PathTraversal(format!(
                "Path traversal attempt: {} resolves outside base directory",
                path
            )));
        }
    }
...
pub fn validate_env_var(name: &str) -> ValidationResult<String> {
    ...
    env::var(name).map_err(|_| {
        ValidationError::InvalidPath(format!("Environment variable not found: {}", name))
    })
}
...
pub fn validate_config_params<T: std::fmt::Debug>(
    ...
    if params.len() > max_config_size {
        return Err(ValidationError::OutOfRange(format!(
            "Configuration too large: {} parameters (max: {})",
            params.len(),
            max_config_size
        )));
    }
...
pub fn validate_config_key(key: &str) -> ValidationResult<()> {
    ...
    if key.len() > 100 {
        return Err(ValidationError::StringTooLong(format!(
            "Configuration key too long: {} characters",
            key.len()
        )));
    }
    ...
    for pattern in &suspicious_patterns {
        if key.to_lowercase().contains(pattern) {
            return Err(ValidationError::InvalidPath(format!(
                "Suspicious pattern in configuration key '{}': {}",
                key, pattern
            )));
        }
    }
    ...
    {
        return Err(ValidationError::InvalidPath(format!(
            "Invalid characters in configuration key: {}",
            key
        )));
    }
...
{
    // Parse the input
    let value = input.trim().parse::<T>().map_err(|e| {
        ValidationError::InvalidPath(format!(
            "Invalid numeric value for '{}': '{}', error: {}",
            field_name, input, e
        ))
    })?;
    ...
    if let Some(min_val) = min {
        if value < min_val {
            return Err(ValidationError::OutOfRange(format!(
                "Value for '{}' must be at least {}, got: {}",
                field_name, min_val, value
            )));
        }
    }
    ...
    if let Some(max_val) = max {
        if value > max_val {
            return Err(ValidationError::OutOfRange(format!(
                "Value for '{}' must be at most {}, got: {}",
                field_name, max_val, value
            )));
        }
    }
...
pub fn validate_url(input: &str, field_name: &str, allow_local: bool) -> ValidationResult<()> {
    ...
    if !has_valid_scheme {
        return Err(ValidationError::InvalidPath(format!(
            "Invalid URL scheme for '{}': {}. Allowed: {}",
            field_name,
            input,
    ...
    for char in dangerous_chars.iter() {
        if trimmed.contains(*char) {
            return Err(ValidationError::InvalidPath(format!(
                "URL contains dangerous character '{}' for '{}'",
                char, field_name
            )));
        }
    }
    ...
    if trimmed.contains("..") && !trimmed.starts_with("http") {
        return Err(ValidationError::PathTraversal(format!(
            "URL contains suspicious patterns for '{}': {}",
            field_name, input
        )));
    }
...
pub fn validate_email(input: &str, field_name: &str) -> ValidationResult<String> {
    ...
    for pattern in &dangerous_patterns {
        if email.to_lowercase().contains(pattern) {
            return Err(ValidationError::InvalidPath(format!(
                "Email contains suspicious pattern for '{}': {}",
                field_name, pattern
            )));
        }
    }
...
pub fn validate_json_string(
    ...
    field_name: &str,
    max_size: usize,
) -> ValidationResult<serde_json::Value> {
    ...
    let json_value: serde_json::Value = serde_json::from_str(&trimmed).map_err(|e| {
        ValidationError::InvalidPath(format!("Invalid JSON for '{}': {}", field_name, e))
    })?;
...
fn validate_json_security(value: &serde_json::Value, field_name: &str) -> ValidationResult<()> {
    ...
    match value {
        Value::String(s) => {
            ...
            for pattern in &dangerous_patterns {
                if s.to_lowercase().contains(pattern) {
                    return Err(ValidationError::InvalidPath(format!(
                        "JSON string contains suspicious pattern in '{}': {}",
                        field_name, pattern
                    )));
                }
            }
            ...
            if s.len() > 10000 {
                return Err(ValidationError::StringTooLong(format!(
                    "JSON string too long in '{}': {} characters",
                    field_name,
                    s.len()
                )));
            }
        ...
        Value::Object(obj) => {
            ...
            if obj.len() > 100 {
                return Err(ValidationError::OutOfRange(format!(
                    "JSON object too large in '{}': {} properties",
                    field_name,
                    obj.len()
                )));
            }
        ...
        Value::Number(n) => {
            ...
            if n.is_i64() && n.as_i64().unwrap_or(0).abs() > 1_000_000_000 {
                return Err(ValidationError::OutOfRange(format!(
                    "JSON number magnitude too large in '{}': {}",
                    field_name, n
                )));
            }
        }
...
pub fn validate_command_args(args: &[String], program_name: &str) -> ValidationResult<Vec<String>> {
    ...
    if args.len() > 50 {
        return Err(ValidationError::OutOfRange(format!(
            "Too many command-line arguments for {}: {} (max: 50)",
            program_name,
            args.len()
        )));
    }
    ...
    for (i, arg) in args.iter().enumerate() {
        ...
        for pattern in &shell_patterns {
            if sanitized.contains(pattern) {
                return Err(ValidationError::InvalidPath(format!(
                    "Command argument {} contains shell injection pattern: '{}'",
                    i, pattern
                )));
            }
        }
...
pub fn validate_buffer_input<'a>(
    buffer: &'a [u8],
    field_name: &str,
    max_size: usize,
) -> ValidationResult<&'a [u8]> {
    // Check buffer size
...
pub fn validate_duration(
    ...
    let duration = if lower.ends_with("ms") {
        ...
        let m = validate_numeric_input(
            &lower[..lower.len() - 1],
            field_name,
            Some(0u64),
            Some(max_duration_ms / (1000 * 60)),
        )?;
        std::time::Duration::from_secs(m * 60)
    } else if lower.ends_with('h') {
        let h = validate_numeric_input(
            &lower[..lower.len() - 1],
            field_name,
            Some(0u64),
            Some(max_duration_ms / (1000 * 60 * 60)),
        )?;
        std::time::Duration::from_secs(h * 60 * 60)
    } else {
        return Err(ValidationError::InvalidPath(format!(
            "Invalid duration format for '{}': '{}'. Use units: ms, s, m, h",
            field_name, input
        )));
    };
...
impl StreamingFileReader {
    ...
    pub fn new(
        ...
        let file = std::fs::File::open(&validated_path).map_err(|e| {
            ValidationError::InvalidPath(format!("Failed to open file {}: {}", path.display(), e))
        })?;
    ...
    pub fn read_next_chunk(&mut self) -> ValidationResult<Option<Vec<u8>>> {
        ...
        match self.file.read(&mut self.buffer) {
            ...
            Err(e) => Err(ValidationError::InvalidPath(format!(
...
mod tests {
    ...
    fn test_validate_numeric_input() {
        ...
        assert_eq!(
            validate_numeric_input::<u32>("42", "test", None, None).unwrap(),
            42
        );
        assert_eq!(
            validate_numeric_input::<i32>("-10", "test", Some(-20), Some(0)).unwrap(),
            -10
        );
    ...
    fn test_validate_email() {
        // Valid emails
        assert_eq!(
            validate_email("user@example.com", "test").unwrap(),
            "user@example.com"
        );
        assert_eq!(
            validate_email("test.email+tag@domain.co.uk", "test").unwrap(),
