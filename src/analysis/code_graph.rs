...
use crate::symbols::{Dependency, DependencyType, Symbol, SymbolKind, DependencyExtractorFactory};
use super::{CodeNode, CodeRelationship, RelationshipKind, CodeMetrics};
...
impl std::fmt::Display for MergeError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            MergeError::IncompatibleSymbols { symbol_name, existing_kind, other_kind } => {
...
impl CodeGraphBuilder {
    ...
    pub fn add_file(
        ...
        let dependencies = self.dependency_extractor.extract_dependencies(
            tree, source, symbols, file_path, language
        );
    ...
    pub fn add_dependency_relationship(&mut self, dependency: Dependency) -> Result<(), String> {
        let from_node = self.find_or_create_external_node(&dependency.from_symbol, &dependency.file_path);
        let to_node = self.find_or_create_external_node(&dependency.to_symbol, &dependency.file_path);
        ...
        let relationship = CodeRelationship {
            ...
            metadata: {
                ...
                meta.insert("dependency_type".to_string(), format!("{:?}", dependency.relationship_type));
                meta.insert("is_conditional".to_string(), dependency.is_conditional.to_string());
    ...
    fn find_or_create_external_node(&mut self, symbol_name: &str, file_path: &str) -> NodeIndex {
        ...
        let node_idx = self.graph.add_node(node_data);
        self.symbol_to_node.insert(symbol_name.to_string(), node_idx);
        node_idx
    }
    ...
    fn analyze_symbol_dependencies(&mut self, from_node: NodeIndex, symbol: &Symbol, file_path: &str) {
        match symbol.kind {
            SymbolKind::Function | SymbolKind::Method => {
                if let Some(signature) = &symbol.signature {
    ...
    fn analyze_function_signature(&mut self, from_node: NodeIndex, signature: &str, file_path: &str) {
    ...
    fn analyze_type_dependencies(&mut self, from_node: NodeIndex, symbol: &Symbol, file_path: &str) {
    ...
    fn create_call_relationship(&mut self, from_node: NodeIndex, called_function: &str, file_path: &str) {
    ...
    fn create_inheritance_relationship(&mut self, from_node: NodeIndex, parent_type: &str, file_path: &str) {
    ...
    fn create_enhanced_import_relationship(&mut self, from_node: NodeIndex, symbol: &Symbol, file_path: &str) {
    ...
    pub fn build_call_graph(&self) -> CallGraphAnalysis {
        ...
        for node_idx in call_graph.node_indices() {
            ...
            let incoming_calls = call_graph.neighbors_directed(node_idx, petgraph::Direction::Incoming).count();
            let outgoing_calls = call_graph.neighbors_directed(node_idx, petgraph::Direction::Outgoing).count();
    ...
    pub fn get_call_chain(&self, source: &str, target: &str) -> Option<Vec<NodeIndex>> {
        if let (Some(&source_idx), Some(&target_idx)) = 
            (self.symbol_to_node.get(source), self.symbol_to_node.get(target)) {
            
            // Use Dijkstra's algorithm to find shortest path in call graph
                
            let call_graph = self.get_call_graph();
            let path_map = petgraph::algo::dijkstra(&call_graph, source_idx, Some(target_idx), |_| 1);
    ...
    pub fn build_import_dependency_graph(&self) -> ImportDependencyAnalysis {
        ...
        for node_idx in import_graph.node_indices() {
            ...
            let imports = import_graph.neighbors_directed(node_idx, petgraph::Direction::Outgoing).count();
            let imported_by = import_graph.neighbors_directed(node_idx, petgraph::Direction::Incoming).count();
    ...
    pub fn get_imports_by_file(&self, file_path: &str) -> Vec<NodeIndex> {
        self.graph
            ...
            .filter(|&idx| {
                ...
                node.symbol.kind == SymbolKind::Import
            })
            .collect()
    }
        ...
        if let Some(&target_idx) = self.symbol_to_node.get(target_module) {
            self.graph
                ...
                .filter(|&importer_idx| {
                    ...
                    self.import_graph_edges.contains(&(importer_idx, target_idx))
                })
                .collect()
        } else {
            Vec::new()
    ...
    pub fn find_imported_modules(&self, source_module: &str) -> Vec<NodeIndex> {
        if let Some(&source_idx) = self.symbol_to_node.get(source_module) {
            self.graph
                ...
                .filter(|&imported_idx| {
                    // Check if there's an import relationship
                    self.import_graph_edges.contains(&(source_idx, imported_idx))
                })
                .collect()
        } else {
            Vec::new()
    ...
    fn build_subgraph(&self, edges: &HashSet<(NodeIndex, NodeIndex)>) -> EnhancedCodeGraph {
        ...
        for &(from_idx, to_idx) in edges {
            ...
            for &node_idx in &[from_idx, to_idx] {
                node_mapping.entry(node_idx).or_insert_with(|| {
                    
                    subgraph.add_node(self.graph[node_idx].clone())
                });
            }
            
            // Add edge
            if let Some(edge_weight) = self.graph.find_edge(from_idx, to_idx)
    ...
    pub fn get_definitions_in_scope(&self, scope: &str) -> Vec<NodeIndex> {
        self.graph
            ...
            .filter(|&idx| {
                let node = &self.graph[idx];
                let symbol_scope = node.symbol.scope_chain
                    .iter()
                    .map(|s| s.name.as_str())
                    .collect::<Vec<_>>()
                    .join("::");
                symbol_scope.starts_with(scope) || 
                (scope.is_empty() && symbol_scope.is_empty())
            })
            .collect()
    }
    ...
    pub fn merge(&mut self, other: CodeGraphBuilder) -> Result<MergeResult, MergeError> {
        ...
        for (symbol_name, &other_node_idx) in &other.symbol_to_node {
            ...
            if let Some(&existing_node_idx) = self.symbol_to_node.get(symbol_name) {
                ...
                self.symbol_to_node.insert(symbol_name.clone(), new_node_idx);
        ...
        for (file_path, other_file_nodes) in &other.file_symbols {
            let mapped_nodes: Vec<NodeIndex> = other_file_nodes.iter()
        ...
        for edge_ref in other.graph.edge_references() {
            ...
            if let (Some(&new_source), Some(&new_target)) = (
                ...
                if self.graph.find_edge(new_source, new_target).is_none() {
                    self.graph.add_edge(new_source, new_target, relationship.clone());
    ...
    fn resolve_symbol_conflict(
        ...
        Ok(SymbolConflict {
            ...
            description: format!(
                ...
                symbol_name,
                existing_symbol.location.file_path,
                other_symbol.location.file_path
            ),
        })
    }
    ...
    fn merge_symbol_information(&mut self, target_node: NodeIndex, source_symbol: &crate::symbols::Symbol) {
    ...
    fn create_renamed_symbol(
        ...
        let new_name = format!("{}_{}", original_name, original.location.file_path.replace(['/', '.'], "_"));
    ...
    fn merge_specialized_edge_sets(
        ...
        for (old_source, old_target) in &other.inheritance_graph_edges {
            if let (Some(&new_source), Some(&new_target)) = (
                ...
                self.inheritance_graph_edges.insert((new_source, new_target));
            }
        }
    ...
    pub fn get_cross_file_dependencies(&self) -> CrossFileDependencies {
        ...
        for edge_ref in self.graph.edge_references() {
            ...
            file_to_file_deps.entry(file_pair.clone())
            ...
            symbol_to_file_deps.entry(source_node.symbol.qualified_name())
                .or_insert_with(Vec::new)
                .push(target_node.file_path.clone());
            
            // Track external dependencies
            if target_node.symbol.modifiers.contains(&"external".to_string()) {
...
mod tests {
    ...
    fn test_symbol_definition_graph() {
        ...
        builder.add_file_symbols(vec![class_symbol, method_symbol, external_symbol], "test.rs");
    ...
    fn test_definition_graph_relationships() {
        ...
        builder.graph.add_edge(child_idx, parent_idx, inheritance_relationship);
    ...
    fn test_call_graph_construction() {
        ...
        // Add all symbols
        builder.add_file_symbols(vec![main_symbol, process_symbol, helper_symbol, cleanup_symbol], "test.rs");
        ...
        builder.graph.add_edge(main_idx, process_idx, call_relationship.clone());

        builder.call_graph_edges.insert((process_idx, helper_idx));
        builder.graph.add_edge(process_idx, helper_idx, call_relationship.clone());

        builder.call_graph_edges.insert((process_idx, cleanup_idx));
        builder.graph.add_edge(process_idx, cleanup_idx, call_relationship);
    ...
    fn test_recursive_call_detection() {
        ...
        let recursive_symbol = Symbol {
            ...
            documentation: None,
            modifiers: vec![],
            signature: Some("fn recursive_func(n: i32) { if n > 0 { recursive_func(n-1); } }".to_string()),
        };
        ...
        builder.graph.add_edge(func_idx, func_idx, recursive_relationship);
    ...
    fn test_import_dependency_graph() {
        ...
        builder.add_file_symbols(vec![main_module, utils_module, external_module], "test_project");
        ...
        builder.graph.add_edge(main_idx, utils_idx, import_rel.clone());
...
impl CodeGraphBuilder {
    ...
    pub fn build_data_flow_graph(&mut self) -> Result<(), String> {
        ...
        let file_paths: Vec<String> = self.graph.node_weights()
    ...
    fn extract_data_flow_from_symbol(
        ...
        let extractor_factory = crate::symbols::dependency_extractor::DependencyExtractorFactory::new();
        
        // Get the actual source content from the file at the symbol's location
        let source_content = if let Ok(content) = std::fs::read_to_string(&symbol.location.file_path) {
            content
        } else {
        ...
        if let Some(parse_result) = parser_factory.parse(&source_content, symbol.language) {
            ...
            for dep in dependencies {
                if matches!(dep.relationship_type, crate::symbols::DependencyType::DataFlow) {
    ...
    fn add_data_flow_edge(
        ...
        let relationship = super::CodeRelationship {
            ...
            source_location: format!("{}:{}:{}", dependency.location.file_path, dependency.location.start_line, dependency.location.start_column),
            confidence: dependency.strength,
            metadata: {
    ...
    pub fn get_data_flow_metrics(&self, symbol_name: &str) -> Option<DataFlowMetrics> {
        if let Some(&node_idx) = self.symbol_to_node.get(symbol_name) {
            let incoming_flows = self.data_flow_edges.iter()
                .filter(|(_, to_idx)| *to_idx == node_idx)
                .count() as u32;
            
            let outgoing_flows = self.data_flow_edges.iter()
    ...
    fn is_in_data_flow_cycle(&self, node_idx: NodeIndex) -> bool {
        ...
        fn has_cycle_dfs(
            ...
            for (from, to) in edges {
                if *from == current {
                    ...
                    if !visited.contains(to) && has_cycle_dfs(*to, target, edges, visited, rec_stack) {
                        return true;
                    }
                }
            }
        ...
        has_cycle_dfs(node_idx, node_idx, &self.data_flow_edges, &mut visited, &mut rec_stack)
    }
    ...
    fn calculate_data_flow_depth(&self, node_idx: NodeIndex) -> u32 {
        ...
        let sources: Vec<NodeIndex> = self.graph.node_indices()
            .filter(|&idx| {
                !self.data_flow_edges.iter().any(|(_, to_idx)| *to_idx == idx)
            })
            .collect();
    ...
    pub fn analyze_data_flow_patterns(&self) -> DataFlowAnalysis {
        ...
        let flow_graph: petgraph::Graph<(), (), petgraph::Directed> = {
            ...
            for &idx in self.data_flow_edges.iter().flat_map(|(a, b)| vec![a, b]) {
                node_map.entry(idx).or_insert_with(|| {
        ...
        analysis.flow_cycles = sccs.into_iter()
        ...
        analysis.complex_flows.sort_by(|a, b| b.complexity_score.cmp(&a.complexity_score));
        analysis.data_hotspots.sort_by(|a, b| b.incoming_flows.cmp(&a.incoming_flows));
...
mod data_flow_tests {
    ...
    fn test_data_flow_graph_construction() {
        ...
        builder.add_data_flow_edge(b_idx, a_idx, &dependency).unwrap();
    ...
    fn test_data_flow_metrics() {
        ...
        builder.add_data_flow_edge(hub_idx, source1_idx, &dep1).unwrap();
        builder.add_data_flow_edge(sink1_idx, hub_idx, &dep2).unwrap();
...
impl CodeGraphBuilder {
    ...
    fn analyze_control_flow_in_file(&mut self, file_path: &str) -> Result<(), String> {
        ...
        for &symbol_idx in &file_symbols {
            let symbol = self.graph[symbol_idx].symbol.clone();
            if matches!(symbol.kind, crate::symbols::SymbolKind::Function | crate::symbols::SymbolKind::Method) {
    ...
    fn extract_control_flow_from_function(
        ...
        if let Some(parse_result) = parser_factory.parse(function_body, function.language) {
            self.traverse_for_control_flow(parse_result.tree.root_node(), function_idx, function_body)?;
        }
    ...
    fn extract_match_control_flow(
        ...
        let mut cursor = node.walk();
        let case_count = node.children(&mut cursor)
    ...
    fn extract_break_continue_control_flow(
        ...
        let stmt_control_node = self.create_control_flow_node(
            &stmt_node_name,
            stmt_kind,
            stmt_text,
            function_idx,
    ...
    pub fn get_control_flow_metrics(&self, function_name: &str) -> Option<ControlFlowMetrics> {
        if let Some(&function_idx) = self.symbol_to_node.get(function_name) {
            let outgoing_flows = self.control_flow_edges.iter()
                .filter(|(from_idx, _)| *from_idx == function_idx)
                .count() as u32;
            
            let incoming_flows = self.control_flow_edges.iter()
    ...
    pub fn analyze_control_flow_patterns(&self) -> ControlFlowAnalysis {
        ...
        for (function_name, &_function_idx) in &self.symbol_to_node {
            if let Some(metrics) = self.get_control_flow_metrics(function_name) {
                ...
                if metrics.exception_count > 2 {
                    analysis.exception_heavy_functions.push(ExceptionHeavyFunction {
                        name: function_name.clone(),
        ...
        analysis.complex_functions.sort_by(|a, b| b.cyclomatic_complexity.cmp(&a.cyclomatic_complexity));
        analysis.deeply_nested_functions.sort_by(|a, b| b.max_nesting_depth.cmp(&a.max_nesting_depth));
        analysis.exception_heavy_functions.sort_by(|a, b| b.exception_count.cmp(&a.exception_count));
...
mod control_flow_tests {
    ...
    fn test_control_flow_graph_construction() {
        ...
        assert!(builder.control_flow_edges.len() > 0);
    }
    ...
    fn test_control_flow_metrics() {
        ...
        let complex_function = Symbol {
            ...
            documentation: None,
            modifiers: vec![],
            signature: Some(r#"fn complex_function() {
                if condition1 {
                    for i in 0..10 {
                        if condition2 {
                            return;
    ...
    fn test_control_flow_paths() {
        ...
        let branching_function = Symbol {
            ...
            documentation: None,
            modifiers: vec![],
            signature: Some(r#"fn branching_function() {
                if condition {
                    return 1;
                } else {
                    return 2;
    ...
    fn test_control_flow_analysis() {
        ...
        let analysis = builder.analyze_control_flow_patterns();
        assert_eq!(analysis.total_control_flows, builder.control_flow_edges.len());
    }
...
impl CodeGraphBuilder {
    ...
    fn validate_graph_structure(&self, result: &mut GraphValidationResult) {
        ...
        let mut scope_symbols: std::collections::HashMap<String, Vec<String>> = std::collections::HashMap::new();
        for (symbol_name, &node_idx) in &self.symbol_to_node {
            if let Some(node) = self.graph.node_weight(node_idx) {
                let scope = node.symbol.scope_chain.iter()
                    .map(|s| s.name.as_str())
                    .collect::<Vec<_>>()
                    .join("::");
                scope_symbols.entry(scope).or_default().push(symbol_name.clone());
            }
        }
    ...
    fn detect_cycles_in_edge_set(&self, edges: &std::collections::HashSet<(NodeIndex, NodeIndex)>) -> Vec<Vec<NodeIndex>> {
        ...
        let nodes: std::collections::HashSet<NodeIndex> = edges.iter()
                ...
                if node.symbol.qualified_name() != *symbol_name && node.symbol.name != *symbol_name {
                    result.errors.push(ValidationError {
                        ...
                        message: format!(
                            ...
                            symbol_name, node.symbol.name, node.symbol.qualified_name()
                        ),
                ...
                if let (Some(from_node), Some(to_node)) = (
                    ...
                    if let Some(relationship) = self.graph.edge_weight(edge_idx) {
                        self.validate_relationship_semantics(
                            from_node, to_node, relationship, result
                        );
                    }
                }
            }
    ...
    fn validate_relationship_semantics(
        ...
        match relationship.kind {
            RelationshipKind::Calls => {
                ...
                if !matches!(from_node.symbol.kind, SymbolKind::Function | SymbolKind::Method) {
                    result.warnings.push(ValidationWarning {
            ...
            RelationshipKind::Imports => {
                ...
                if !matches!(
                    to_node.symbol.kind,
                    SymbolKind::Module | SymbolKind::Namespace | SymbolKind::Class | 
                    SymbolKind::Function | SymbolKind::Type | SymbolKind::Constant
                ) {
                    result.warnings.push(ValidationWarning {
                        kind: ValidationWarningKind::SemanticMismatch,
                        message: format!(
            ...
            if !has_incoming && !has_outgoing {
                if let Some(node) = self.graph.node_weight(node_idx) {
                    result.warnings.push(ValidationWarning {
                        ...
                        message: format!("Orphaned node with no relationships: '{}'", node.symbol.name),
                        location: Some(node.symbol.location.clone()),
                    });
                }
            }
    ...
    fn validate_scope_consistency(&self, result: &mut GraphValidationResult) {
        for (symbol_name, &node_idx) in &self.symbol_to_node {
            if let Some(node) = self.graph.node_weight(node_idx) {
                ...
                let scope_from_chain = node.symbol.scope_chain.iter()
                    .map(|s| s.name.as_str())
                    .collect::<Vec<_>>()
                    .join("::");
                let qualified_name = node.symbol.qualified_name();
    ...
    pub fn analyze_cycles(&self) -> CycleAnalysis {
        ...
        for cycle in &validation_result.cycles {
            *analysis.cycles_by_kind.entry(cycle.kind).or_insert(0) += 1;
            ...
            if matches!(cycle.severity, CycleSeverity::Error | CycleSeverity::Warning) {
        ...
        let cycle_lengths: Vec<usize> = validation_result.cycles.iter()
            .map(|c| c.nodes.len())
            .collect();
    ...
    fn describe_cycle(&self, cycle: &GraphCycle) -> String {
        let symbol_names: Vec<String> = cycle.nodes.iter()
            .filter_map(|&node_idx| {
    ...
    pub fn has_critical_issues(&self) -> bool {
        ...
        validation_result.cycles.iter().any(|c| matches!(c.severity, CycleSeverity::Error))
    }
    ...
    pub fn get_health_report(&self) -> GraphHealthReport {
        ...
        GraphHealthReport {
            ...
            error_count,
            warning_count,
            critical_cycle_count: cycle_analysis.problematic_cycles.iter()
    ...
    fn calculate_health_score(&self, validation: &GraphValidationResult, cycles: &CycleAnalysis) -> f32 {
        let mut score = 1.0;
        ...
        let critical_cycles = cycles.problematic_cycles.iter()
            .filter(|c| matches!(c.severity, CycleSeverity::Error))
            .count();
        score -= critical_cycles as f32 * 0.15;
        
        // Penalize warning cycles lightly
        let warning_cycles = cycles.problematic_cycles.iter()
    ...
    fn generate_recommendations(&self, validation: &GraphValidationResult, cycles: &CycleAnalysis) -> Vec<String> {
        let mut recommendations = Vec::new();
        
        if !validation.errors.is_empty() {
            recommendations.push(format!("Fix {} critical errors in graph structure", validation.errors.len()));
        }
        
        if validation.warnings.len() > 10 {
            recommendations.push(format!("Address {} warnings to improve code quality", validation.warnings.len()));
        }
        
        let import_cycles = cycles.cycles_by_kind.get(&CycleKind::ImportCycle).unwrap_or(&0);
        if *import_cycles > 0 {
            recommendations.push(format!("Resolve {import_cycles} circular import dependencies"));
        }
        
        let inheritance_cycles = cycles.cycles_by_kind.get(&CycleKind::InheritanceCycle).unwrap_or(&0);
        if *inheritance_cycles > 0 {
            recommendations.push(format!("Fix {inheritance_cycles} circular inheritance relationships"));
        }
        ...
        if validation.warnings.iter().any(|w| matches!(w.kind, ValidationWarningKind::OrphanedNode)) {
...
mod validation_tests {
    ...
    fn test_cycle_detection() {
        ...
        let call_cycles: Vec<_> = validation_result.cycles.iter()
    ...
    fn test_import_cycle_detection() {
        ...
        let validation_result = builder.validate_graph();
        let import_cycles: Vec<_> = validation_result.cycles.iter()
    ...
    fn test_orphaned_node_detection() {
        ...
        builder.call_graph_edges.insert((connected_idx, orphaned_idx));
        builder.graph.add_edge(connected_idx, orphaned_idx, call_rel);

        // Validate and check for orphaned nodes
        let validation_result = builder.validate_graph();
        
        // connected_function should be orphaned (no incoming edges)
        let orphaned_warnings: Vec<_> = validation_result.warnings.iter()
    ...
    fn test_cycle_analysis() {
        ...
        assert!(cycle_analysis.cycles_by_kind.contains_key(&CycleKind::InheritanceCycle));
        assert!(!cycle_analysis.problematic_cycles.is_empty());
        
        // Inheritance cycles should be marked as errors
        let inheritance_cycle = cycle_analysis.problematic_cycles.iter()
            .find(|c| matches!(c.kind, CycleKind::InheritanceCycle));
        assert!(inheritance_cycle.is_some());
        assert!(matches!(inheritance_cycle.unwrap().severity, CycleSeverity::Error));
    }
...
mod merge_tests {
    use super::*;
    use crate::symbols::{Symbol, SymbolKind, Location, Scope};
    use crate::parsers::LanguageId;
    ...
    fn test_symbol_conflict_resolution() {
        ...
        assert!(matches!(conflict.resolution_strategy, ConflictResolutionStrategy::KeepExisting));
    }
    ...
    fn test_external_vs_actual_symbol_resolution() {
        ...
        let external_symbol = create_external_symbol("shared_function", SymbolKind::Function, "external");
    ...
    fn test_incompatible_symbol_types() {
        ...
        if let Err(MergeError::IncompatibleSymbols { symbol_name, existing_kind, other_kind }) = merge_result {
    ...
    fn test_merge_multiple_builders() {
        ...
        builder1.add_file_symbols(vec![
            create_test_symbol("func1", SymbolKind::Function, "file1.rs")
        ], "file1.rs");

        builder2.add_file_symbols(vec![
            create_test_symbol("func2", SymbolKind::Function, "file2.rs")
        ], "file2.rs");

        builder3.add_file_symbols(vec![
            create_test_symbol("func3", SymbolKind::Function, "file3.rs")
        ], "file3.rs");
    ...
    fn test_cross_file_dependencies_analysis() {
        ...
        let external_symbol = create_external_symbol("external_lib", SymbolKind::Function, "external");
        ...
        builder.graph.add_edge(caller_node, callee_node, call_relationship);
        builder.graph.add_edge(caller_node, external_node, external_relationship);
        ...
        assert!(cross_file_deps.file_to_file.contains_key(&("file1.rs".to_string(), "file2.rs".to_string())));
    ...
    fn test_rename_strategy_for_different_languages() {
        ...
        let conflict = &merge_result.conflicts[0];
        assert!(matches!(conflict.resolution_strategy, ConflictResolutionStrategy::Rename));
    }
    ...
    fn test_merge_with_relationships() {
        ...
        assert!(builder1.graph.find_edge(merged_c_node, merged_d_node).is_some());
    }
}