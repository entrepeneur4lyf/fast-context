...
impl PyJsonExporter {
    ...
    pub fn export_analysis(&self, analysis: &AnalysisResult) -> PyResult<String> {
        ...
        if self.options.include_relationships {
            let relationships_json: Vec<Value> = analysis
                .relationships
                .iter()
                .map(|rel| {
    ...
    fn generate_embeddings(&self, analysis: &AnalysisResult) -> String {
        ...
        embeddings.insert(
            "embedding_format".to_string(),
            json!(self.options.embedding_format),
        );
...
impl PyLspExporter {
    ...
    ) -> PyResult<Vec<PyLspSymbolInformation>> {
