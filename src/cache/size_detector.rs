...
impl CodebaseAnalyzer {
    ...
    fn calculate_complexity_score(&self, contents: &str) -> f64 {
        ...
        let nesting_chars = contents
            .chars()
            .filter(|&c| c == '{' || c == '(' || c == '[')
            .count();
        let control_keywords = [
            "if", "else", "for", "while", "match", "switch", "try", "catch",
        ]
        .iter()
    ...
    fn classify_project_type(
        ...
        if files
            .iter()
            .any(|f| f.path.to_string_lossy().contains("package.json"))
        {
            return ProjectType::WebApp;
        }

        if languages.contains_key(&LanguageId::Swift)
            || languages.contains_key(&LanguageId::ObjectiveC)
        {
            return ProjectType::MobileApp;
        }

        if languages.contains_key(&LanguageId::Rust)
            && files
                .iter()
    ...
    fn estimate_parse_time(
        &self,
        files: &[FileInfo],
        _languages: &HashMap<LanguageId, LanguageStats>,
    ) -> u64 {
    ...
    pub fn cache_recommendations(&self) -> CacheRecommendations {
        CacheRecommendations {
            ...
            enable_predictive: matches!(
                self.size,
                ProjectSize::Medium | ProjectSize::Large | ProjectSize::Massive
            ),
