...
mod tests {
    ...
    fn test_policy_type_from_project_size() {
        assert_eq!(CachePolicyType::from_project_size(ProjectSize::Tiny), CachePolicyType::Minimal);
        assert_eq!(CachePolicyType::from_project_size(ProjectSize::Small), CachePolicyType::Balanced);
        assert_eq!(CachePolicyType::from_project_size(ProjectSize::Medium), CachePolicyType::Adaptive);
        assert_eq!(CachePolicyType::from_project_size(ProjectSize::Large), CachePolicyType::Persistent);
        assert_eq!(CachePolicyType::from_project_size(ProjectSize::Massive), CachePolicyType::Enterprise);
    }
    ...
    fn test_config_from_tiny_project() {
        ...
        fs::write(temp_path.join("main.rs"), "fn main() { println!(\"Hello\"); }").unwrap();
        fs::write(temp_path.join("lib.rs"), "pub fn hello() -> String { \"world\".to_string() }").unwrap();
        fs::write(temp_path.join("utils.rs"), "pub fn add(a: i32, b: i32) -> i32 { a + b }").unwrap();
