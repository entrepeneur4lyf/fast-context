...
// Simple prompt argument types
interface CodeReviewPromptArgs {
  filePath: string;
  focusAreas?: string[];
  severity?: string;
}

interface RefactoringSuggestionsPromptArgs {
  symbolName: string;
  refactoringType?: string;
  includeExamples?: boolean;
}

interface ArchitectureAnalysisPromptArgs {
  projectPath: string;
  analysisType?: string;
  includeRecommendations?: boolean;
}

interface DocumentationGenerationPromptArgs {
  symbolName: string;
  format?: string;
  includeExamples?: boolean;
}
...
export function registerPrompts(server: McpServer): void {
  ...
  server.registerPrompt(
    ...
    async (args: any) => {
      const { filePath, focusAreas, severity = 'medium' } = args as CodeReviewPromptArgs;
      try {
...
- Performance optimization opportunities`
  ...
  server.registerPrompt(
    ...
    async (args: any) => {
      const { symbolName, refactoringType, includeExamples = true } = args as RefactoringSuggestionsPromptArgs;
      try {
...
- Testing strategies`
  ...
  server.registerPrompt(
    ...
    async (args: any) => {
      const { projectPath, analysisType = 'overview', includeRecommendations = true } = args as ArchitectureAnalysisPromptArgs;
      try {
...
- Potential improvements`
  ...
  server.registerPrompt(
    ...
    async (args: any) => {
      const { symbolName, format = 'markdown', includeExamples = true } = args as DocumentationGenerationPromptArgs;
      try {
        const formatText = {
