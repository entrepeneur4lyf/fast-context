...
interface GetSymbolContextInput {
  projectPath: string;
  symbolName: string;
...
interface GetComplexityMetricsInput {
  projectPath: string;
  filePath?: string;
  includeDetails?: boolean;
}

interface QuerySemanticInput {
  projectPath: string;
  query: string;
  limit?: number;
  similarity?: number;
...
export function registerTools(server: McpServer): void {
  ...
  server.registerTool(
    ...
    async (args: any) => {
      const { projectPath, options } = args as AnalyzeCodebaseInput;
      try {
  ...
  server.registerTool(
    ...
    async (args: any) => {
      const { projectPath, query, kind, limit = 50 } = args as FindSymbolsInput;
      try {
        return {
          content: [{
            type: 'text',
  ...
  server.registerTool(
    ...
    async (args: any) => {
      const { projectPath, symbolName, includeReferences = true, depth = 3 } = args as GetSymbolContextInput;
      try {
        return {
          content: [{
            type: 'text',
  ...
  server.registerTool(
    ...
    async (args: any) => {
      const { projectPath, symbolName, depth = 5, includeExternal = false } = args as AnalyzeDependenciesInput;
      try {
        return {
          content: [{
            type: 'text',
  ...
  server.registerTool(
    ...
    async (args: any) => {
      const { projectPath, patterns, includeRecommendations = true } = args as DetectPatternsInput;
      try {
        return {
          content: [{
            type: 'text',
  ...
  server.registerTool(
    ...
    async (args: any) => {
      const { projectPath, filePath, includeDetails = true } = args as GetComplexityMetricsInput;
      try {
        return {
          content: [{
            type: 'text',
  ...
  server.registerTool(
    ...
    async (args: any) => {
      const { projectPath, query, limit = 20, similarity = 0.7 } = args as QuerySemanticInput;
      try {
        return {
          content: [{
            type: 'text',
