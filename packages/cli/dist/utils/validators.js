 ...
 */
export async function validateOutputPath(path) {
    if (!path || typeof path !== 'string') {
 ...
 */
export function validateNumber(value, name, min, max) {
...
export function validateFormat(format) {
    const validFormats = ['table', 'json', 'yaml', 'markdown'];
    const normalizedFormat = format.toLowerCase().trim();
    if (!validFormats.includes(normalizedFormat)) {
        throw new ValidationError(`Invalid format "${format}". Supported formats: ${validFormats.join(', ')}`);
    }
 ...
 */
export function validateSymbolName(name) {
