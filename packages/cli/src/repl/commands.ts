...
export class ReplCommands {
  constructor(private repl: FastContextRepl) {}
  ...
  async dependencies(args: string[]): Promise<void> {
    if (!args[0]) {
  ...
  async metrics(args: string[]): Promise<void> {
    const analysis = this.repl.getCurrentAnalysis();
    if (!analysis) {
      console.log(chalk.yellow('No analysis available. Run "analyze" first.'));
      return;
  ...
  async export(args: string[]): Promise<void> {
    ...
    try {
      const formatted = await formatAnalysisResults(analysis, { format: format as any });
  ...
  async config(args: string[]): Promise<void> {
...
`
  ...
  private parseDepthOption(args: string[]): number {
