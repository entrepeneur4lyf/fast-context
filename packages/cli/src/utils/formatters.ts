...
export async function formatAnalysisResults(
...
function formatAsTable(results: AnalysisResults, options: FormatOptions): string {
  const output: string[] = [];
  ...
  summaryTable.push(
    ['Files Analyzed', chalk.white(results.fileCount.toString())],
    ['Symbols Found', chalk.white(results.symbolCount.toString())],
    ['Relationships', chalk.white(results.relationshipCount.toString())],
    ['Languages', chalk.white(results.languages.join(', '))],
    ['Duration', chalk.white(`${results.durationMs}ms`)]
  );
  
  if (results.performance) {
    summaryTable.push(
      ['Memory Usage', chalk.white(`${results.performance.memoryUsageMb}MB`)],
      ['CPU Usage', chalk.white(`${results.performance.cpuUsagePercent}%`)],
      ['Throughput', chalk.white(`${results.performance.throughputFilesPerSecond} files/sec`)]
    );
  }
  ...
  if (options.verbose && results.symbols && results.symbols.length > 0) {
    ...
    results.symbols.slice(0, 10).forEach((symbol: any) => {
      symbolsTable.push([
  ...
  if (results.insights && results.insights.length > 0) {
    ...
    results.insights.forEach((insight: any, index: number) => {
...
function formatAsMarkdown(results: AnalysisResults, options: FormatOptions): string {
  const output: string[] = [];
  ...
  if (results.performance) {
    summaryData.push(
      ['Memory Usage', `${results.performance.memoryUsageMb}MB`],
      ['CPU Usage', `${results.performance.cpuUsagePercent}%`],
      ['Throughput', `${results.performance.throughputFilesPerSecond} files/sec`]
    );
  }
  ...
  if (options.verbose && results.symbols && results.symbols.length > 0) {
    ...
    const symbolsData = [
      ['Name', 'Type', 'File', 'Line'],
      ...results.symbols.slice(0, 10).map((symbol: any) => [
        symbol.name,
        symbol.kind,
        symbol.file,
...
export function formatSymbolList(symbols: any[], options: FormatOptions): string {
  if (options.json) {
...
export function formatDependencyGraph(dependencies: any[], options: FormatOptions): string {
  if (options.json) {
