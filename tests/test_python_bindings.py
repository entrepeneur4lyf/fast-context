...
""")
    ...
    def test_parser_based_symbols_in_file(self, temp_project):
        """Verify parser-based symbol extraction returns kind:name strings."""
        config = AnalyzerConfig(
            project_root=temp_project,
            languages=["rust", "python"],
            enable_caching=False,
        ...
        analyzer = FastContextAnalyzer(config)
        analyzer.analyze()
        # Use class method async path for per-file extraction parity
        symbols = analyzer.find_symbols_in_file("main.rs")
        assert isinstance(symbols, list)
        assert any(s.startswith("function: ") or s.startswith("class: ") for s in symbols)
