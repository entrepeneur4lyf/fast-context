...
mod integration_tests {
    ...
    fn test_input_validation() {
        ...
        let valid_kinds = vec![
            "function", "class", "interface", "module", "variable", 
            "constant", "enum", "struct", "trait", "type_alias"
        ];
        
        for kind in valid_kinds {
            assert!(is_valid_symbol_kind(kind), "Should accept valid symbol kind: {}", kind);
        }
        
        // Test invalid symbol kinds
        let long_string = "a".repeat(501);
        let invalid_kinds = vec![
            "", " ", "invalid_kind", "function; DROP TABLE",
            &long_string, "function\0null", "<script>alert('xss')</script>"
        ];

        for kind in invalid_kinds {
            assert!(!is_valid_symbol_kind(kind), "Should reject invalid symbol kind: {}", kind);
        }
    }
    ...
    fn test_path_validation() {
        ...
        let valid_paths = vec![
            ...
            "./src/main.rs", "src/utils/helper.rs"
        ];
        ...
        let malicious_paths = vec![
            ...
            "src/main.rs\0.txt", "src/main.rs; rm -rf /"
        ];
        
        for path in malicious_paths {
            assert!(!is_valid_file_path(path), "Should reject malicious path: {}", path);
        }
    }
    ...
    fn test_string_processing() {
        ...
        for test_string in test_strings {
            ...
            assert!(!processed.is_empty(), "Should process string: {}", test_string);
            assert!(processed.len() <= test_string.len(), "Processed string should not be longer");
        }
    }
    ...
    fn test_cache_operations() {
        ...
        assert_eq!(cache.len(), initial_size + 10, "Cache should contain all inserted entries");
    }
    ...
    fn test_performance_characteristics() {
        ...
        assert!(duration.as_millis() < 1000, "Operation should complete within 1 second");
    }
