...
fn test_invalid_language_configuration() {
    ...
    assert!(
        result.is_ok(),
        "Should handle invalid language configuration"
    );
}
...
fn test_query_nonexistent_symbol() {
    ...
    assert!(
        symbols.is_empty(),
        "Should return empty list for nonexistent symbol kind"
    );

    let result = analyzer.find_symbols_in_file(
        temp_path
            .join("nonexistent_file.rs")
            .to_string_lossy()
            .to_string(),
    );
    ...
    assert!(
        deps.is_empty(),
        "Should return empty list for nonexistent symbol"
    );
}
...
fn test_query_with_invalid_inputs() {
    ...
    assert!(
        result.is_ok(),
        "Very long symbol kind should not cause panic"
    );

    let result = analyzer.find_dependencies(long_string);
    assert!(
        result.is_ok(),
        "Very long symbol name should not cause panic"
    );
}
...
fn test_error_conversion_consistency() {
    ...
    assert!(
        ...
        "Error message should contain original context"
    );
}
...
fn test_recovery_after_errors() {
    ...
    assert!(
        result.is_ok(),
        "Analyzer should remain functional after errors"
    );
    ...
    assert!(
        result.is_ok(),
        "Analyzer should recover from multiple errors"
    );
}
