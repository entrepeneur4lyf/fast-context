...
mod integration_tests {
    ...
    fn test_symbol_extraction_across_languages() {
        ...
        for (file_path, expected_symbols) in test_files {
            ...
            assert!(symbols.len() >= expected_symbols,
                   "Expected at least {} symbols, got {}", expected_symbols, symbols.len());
        }
    }
    ...
    fn test_language_detection_integration() {
        ...
        for (filename, expected_lang) in test_cases {
            ...
            if let Some(detected_str) = detected {
                ...
                assert_eq!(detected_lang, Some(expected_lang),
    ...
    fn create_multi_language_project(base_path: &std::path::Path) {
        ...
        // JavaScript file
        fs::write(base_path.join("app.js"), r#"
            function greet(name) {
                return `Hello, ${name}!`;
            }
        ...
        // Python file
        fs::write(base_path.join("script.py"), r#"
            def hello_world():
                return "Hello, World!"
        ...
        // TypeScript file
        fs::write(base_path.join("component.ts"), r#"
            interface User {
                name: string;
                age: number;
            }
